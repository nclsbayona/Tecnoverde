...
type PedirDatosLogin struct {
	focusIndex int
	title      string
	inputs     []textinput.Model
	cursorMode cursor.Mode
	funcion    func(string, string) (Interfaces.ModeloDeDatos, error)
}

func startModelPedirDatosLogin(fields []string, title string, funcion func(string, string) (Interfaces.ModeloDeDatos, error)) PedirDatosLogin {
	m := PedirDatosLogin{
...
func (m PedirDatosLogin) Init() tea.Cmd {
	return textinput.Blink
}

func (m PedirDatosLogin) Update(msg tea.Msg) (tea.Model, tea.Cmd) {
	switch msg := msg.(type) {
	case tea.KeyMsg:
		switch msg.String() {
		default:
		...
		case "tab", "shift+tab", "enter", "up", "down":
			...
			if s == "enter" && m.focusIndex == len(m.inputs) {
				result, err := m.funcion(m.inputs[0].Value(), m.inputs[1].Value())
...
func (m *PedirDatosLogin) updateInputs(msg tea.Msg) tea.Cmd {
	cmds := make([]tea.Cmd, len(m.inputs))
...
func (m PedirDatosLogin) View() string {
...
func NewPedirDatosLogin(fields []string, title string, funcion func(string, string) (Interfaces.ModeloDeDatos, error)) PedirDatosLogin {
	return startModelPedirDatosLogin(fields, title, funcion)
}

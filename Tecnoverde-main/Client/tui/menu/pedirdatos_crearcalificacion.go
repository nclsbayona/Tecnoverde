...
type PedirDatosCrearCalificacion struct {
	focusIndex      int
	title           string
	inputs          []textinput.Model
	cursorMode      cursor.Mode
	funcion_general func(string, string, string, string) (Interfaces.ModeloDeDatos, error)
}

func startModelPedirDatosCrearCalificacion(fields []string, title string, funcion_general func(string, string, string, string) (Interfaces.ModeloDeDatos, error)) PedirDatosCrearCalificacion {
	m := PedirDatosCrearCalificacion{
...
func (m PedirDatosCrearCalificacion) Init() tea.Cmd {
	return textinput.Blink
}

func (m PedirDatosCrearCalificacion) Update(msg tea.Msg) (tea.Model, tea.Cmd) {
	switch msg := msg.(type) {
	case tea.KeyMsg:
		switch msg.String() {
		default:
		...
		case "tab", "shift+tab", "enter", "up", "down":
			...
			if s == "enter" && m.focusIndex == len(m.inputs) {
				if m.inputs[0].Value() != "" && m.inputs[1].Value() != "" && m.inputs[2].Value() != "" {
					result, err := m.funcion_general(m.inputs[0].Value(), m.inputs[1].Value(), m.inputs[2].Value(), m.inputs[3].Value())
...
func (m *PedirDatosCrearCalificacion) updateInputs(msg tea.Msg) tea.Cmd {
	cmds := make([]tea.Cmd, len(m.inputs))
...
func (m PedirDatosCrearCalificacion) View() string {
...
func NewPedirDatosCrearCalificacion(fields []string, title string, funcion_general func(string, string, string, string) (Interfaces.ModeloDeDatos, error)) PedirDatosCrearCalificacion {
	return startModelPedirDatosCrearCalificacion(fields, title, funcion_general)
}

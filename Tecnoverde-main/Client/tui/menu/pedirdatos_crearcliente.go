...
type PedirDatosCrearCliente struct {
	focusIndex int
	title      string
	inputs     []textinput.Model
	cursorMode cursor.Mode
	funcion    func(string, string, string, string, string, string) (Interfaces.ModeloDeDatos, error)
}

func startModelPedirDatosCrearCliente(fields []string, title string, funcion func(string, string, string, string, string, string) (Interfaces.ModeloDeDatos, error)) PedirDatosCrearCliente {
	m := PedirDatosCrearCliente{
...
func (m PedirDatosCrearCliente) Init() tea.Cmd {
	return textinput.Blink
}

func (m PedirDatosCrearCliente) Update(msg tea.Msg) (tea.Model, tea.Cmd) {
	switch msg := msg.(type) {
	case tea.KeyMsg:
		switch msg.String() {
		default:
		...
		case "tab", "shift+tab", "enter", "up", "down":
			...
			if s == "enter" && m.focusIndex == len(m.inputs) {
				result, err := m.funcion(m.inputs[0].Value(), m.inputs[1].Value(), m.inputs[2].Value(), m.inputs[3].Value(), m.inputs[4].Value(), m.inputs[5].Value())
...
func (m *PedirDatosCrearCliente) updateInputs(msg tea.Msg) tea.Cmd {
	cmds := make([]tea.Cmd, len(m.inputs))
...
func (m PedirDatosCrearCliente) View() string {
...
func NewPedirDatosCrearCliente(fields []string, title string, funcion func(string, string, string, string, string, string) (Interfaces.ModeloDeDatos, error)) PedirDatosCrearCliente {
	return startModelPedirDatosCrearCliente(fields, title, funcion)
}

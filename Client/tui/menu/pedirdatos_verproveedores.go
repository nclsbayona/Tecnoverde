...
type PedirDatosVerProveedores struct {
	focusIndex      int
	title           string
	inputs          []textinput.Model
	cursorMode      cursor.Mode
	funcion_general func() ([]Interfaces.ModeloDeDatos, error)
	funcion_unica   func(int) (Interfaces.ModeloDeDatos, error)
}

func startModelPedirDatosVerProveedores(fields []string, title string, funcion func() ([]Interfaces.ModeloDeDatos, error), funcion_unica func(int) (Interfaces.ModeloDeDatos, error)) PedirDatosVerProveedores {
	m := PedirDatosVerProveedores{
		inputs:          make([]textinput.Model, len(fields)),
		title:           title,
		funcion_general: funcion,
		funcion_unica:   funcion_unica,
	}
...
func (m PedirDatosVerProveedores) Init() tea.Cmd {
	return textinput.Blink
}

func (m PedirDatosVerProveedores) Update(msg tea.Msg) (tea.Model, tea.Cmd) {
	switch msg := msg.(type) {
	case tea.KeyMsg:
		switch msg.String() {
		default:
		...
		case "tab", "shift+tab", "enter", "up", "down":
			...
			if s == "enter" && m.focusIndex == len(m.inputs) {
				if m.inputs[0].Value() == "" {
					...
					result, err := m.funcion_unica(val)
					if err == nil && result != nil {
...
func (m *PedirDatosVerProveedores) updateInputs(msg tea.Msg) tea.Cmd {
	cmds := make([]tea.Cmd, len(m.inputs))
...
func (m PedirDatosVerProveedores) View() string {
...
func NewPedirDatosVerProveedores(fields []string, title string, funcion func() ([]Interfaces.ModeloDeDatos, error), funcion_unica func(int) (Interfaces.ModeloDeDatos, error)) PedirDatosVerProveedores {
	return startModelPedirDatosVerProveedores(fields, title, funcion, funcion_unica)
}

...
type PedirDatosCrearServicio struct {
	focusIndex      int
	title           string
	inputs          []textinput.Model
	cursorMode      cursor.Mode
	funcion_general func(string, string, string, string, string, map[string]string) (Interfaces.ModeloDeDatos, error)
}

func startModelPedirDatosCrearServicio(fields []string, title string, funcion_general func(string, string, string, string, string, map[string]string) (Interfaces.ModeloDeDatos, error)) PedirDatosCrearServicio {
	m := PedirDatosCrearServicio{
...
func (m PedirDatosCrearServicio) Init() tea.Cmd {
	return textinput.Blink
}

func (m PedirDatosCrearServicio) Update(msg tea.Msg) (tea.Model, tea.Cmd) {
	switch msg := msg.(type) {
	case tea.KeyMsg:
		switch msg.String() {
		default:
		...
		case "tab", "shift+tab", "enter", "up", "down":
			...
			if s == "enter" && m.focusIndex == len(m.inputs) {
				...
				result, err := m.funcion_general(m.inputs[0].Value(), m.inputs[1].Value(), m.inputs[2].Value(), m.inputs[3].Value(), m.inputs[4].Value(), map[string]string{"Ubicacion": m.inputs[5].Value()})
...
func (m *PedirDatosCrearServicio) updateInputs(msg tea.Msg) tea.Cmd {
	cmds := make([]tea.Cmd, len(m.inputs))
...
func (m PedirDatosCrearServicio) View() string {
...
func NewPedirDatosCrearServicio(fields []string, title string, funcion_general func(string, string, string, string, string, map[string]string) (Interfaces.ModeloDeDatos, error)) PedirDatosCrearServicio {
	return startModelPedirDatosCrearServicio(fields, title, funcion_general)
}
